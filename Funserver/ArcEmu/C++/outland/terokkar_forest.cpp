...
struct npc_xiriAI : public ScriptedAI
{
      ...
      switch(uiPhase)
	{
	  ...
	  break;
	  case 1:
	    if (Creature* uiAkama = me->SummonCreature(NPC_AKAMA,-3561.79,666.46,-1.39,1.57,TEMPSUMMON_CORPSE_DESPAWN,30000))
	    {
	        me->AddThreat(uiAkama, 0.0f);
		AkamaGUID = uiAkama->GetGUID();
	    }
	    if (Creature* uiMayev = me->SummonCreature(NPC_MAYEV,-3567.29,662.16,-1.39,1.57,TEMPSUMMON_CORPSE_DESPAWN,30000))
	    {
	        me->AddThreat(uiMayev, 0.0f);
		MayevGUID = uiMayev->GetGUID();
	    }
	    NextStep(4000,true);
	    break;
	  case 2:
	  me->SetVisibility(VISIBILITY_OFF);
	  me->GetMotionMaster()->MovePoint(0, -3564.41,444.32,26.72);
	    if (Creature* uiAkama = Unit::GetCreature(*me, AkamaGUID))
	    {
		uiAkama->GetMotionMaster()->MovePoint(0, -3564.41,444.32,26.72);
		uiAkama->SetReactState(REACT_AGGRESSIVE);
		}
	    if (Creature* uiMayev = Unit::GetCreature(*me, MayevGUID))
	    {
		uiMayev->GetMotionMaster()->MovePoint(0, -3563.41,441.32,26.72);
		uiMayev->SetReactState(REACT_AGGRESSIVE);
		}
	    NextStep(20000,true);
	    break;
	  case 3:
		me->GetMotionMaster()->MovePoint(0, -3571.53,335.87,36.85);
	    if (Creature* uiAkama = Unit::GetCreature(*me, AkamaGUID))
		uiAkama->GetMotionMaster()->MovePoint(0, -3571.53,335.87,36.85);
	    if (Creature* uiMayev = Unit::GetCreature(*me, MayevGUID))
		uiMayev->GetMotionMaster()->MovePoint(0, -3574.53,331.87,36.85);
	    NextStep(10000,true);
	    break;
	  case 4:
	  me->GetMotionMaster()->MovePoint(0, -9652.12,319.77,36.24);
	    if (Creature* uiAkama = Unit::GetCreature(*me, AkamaGUID))
		uiAkama->GetMotionMaster()->MovePoint(0, -9652.12,319.77,36.24);
	    if (Creature* uiMayev = Unit::GetCreature(*me, MayevGUID))
		uiMayev->GetMotionMaster()->MovePoint(0, -9654.12,316.77,36.24);
	    NextStep(10000,true);
	    break;
	  case 5:
	    if (Creature* uiAkama = Unit::GetCreature(*me, AkamaGUID))
		Completed = true;
		if (pPlayer->hasQuest(QUEST_10985) && pPlayer->hasQuest(QUEST_13429))
		{
        pPlayer->AreaExploredOrEventHappens(QUEST_10985);
		pPlayer->AreaExploredOrEventHappens(QUEST_13429);
		}
		NextStep(5000,true);
                break;
	  case 6:
	  me->GetMotionMaster()->MovePoint(0,-3561.79,666.46,-1.39);
	    if (Creature* uiAkama = Unit::GetCreature(*me, AkamaGUID))
	        uiAkama->RemoveFromWorld();
	    if (Creature* uiMayev = Unit::GetCreature(*me, MayevGUID))
		uiMayev->RemoveFromWorld();
...
bool GossipHello_npc_xiri(Player* pPlayer, Creature* pCreature)
{
if (pCreature->isQuestGiver())
        pPlayer->PrepareQuestMenu(pCreature->GetGUID());

if (pPlayer->GetQuestStatus(QUEST_10985) == QUEST_STATUS_INCOMPLETE || pPlayer->GetQuestStatus(QUEST_13429) == QUEST_STATUS_INCOMPLETE)
...
bool GossipSelect_npc_xiri(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF+1)
    {
        pPlayer->CLOSE_GOSSIP_MENU();
	CAST_AI(npc_xiriAI, pCreature->AI())->Startquest();
    }
    return true;
}

CreatureAI* GetAI_npc_xiri(Creature* pCreature)
{
