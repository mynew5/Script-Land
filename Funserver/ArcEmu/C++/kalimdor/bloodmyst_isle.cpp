...
CreatureAI* GetAI_npc_princess_stillpineAI(Creature* pCreature)
{
    return new npc_princess_stillpineAI(pCreature);
}

bool go_stillpine_cage(Player* pPlayer, GameObject* pGo)
{
    if(pPlayer->GetQuestStatus(QUEST_SAVE_PRINCESS_STILLPINE) == QUEST_STATUS_INCOMPLETE)
    {
        ...
        if(pCreature)
        {
            ...
            CAST_AI(npc_princess_stillpineAI, pCreature->AI())->FleeTimer = 3500;
            return false;
        }
    }
    return true;
...
void AddSC_bloodmyst_isle()
{
    ...
    newscript->pGOHello = &go_stillpine_cage;
    newscript->RegisterSelf();
}
