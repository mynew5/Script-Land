...
/*######
## npc_totem_binary 22444
######*/
enum tbinary
{
   NPC_DEFENSIVE = 16878,
   NCP_SPIRIT_DEFENSIVE = 22454,
   SPELL_LUCH = 39184
};

struct npc_totem_binaryAI : public ScriptedAI
{
    npc_totem_binaryAI(Creature* pCreature) : ScriptedAI(pCreature) {}
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        if (uiCast = 1)
        {
            if (Creature* pTarget = me->FindNearestCreature(NPC_DEFENSIVE, 10.0f))
	      {
	        me->CastSpell(pTarget,SPELL_LUCH, true); //TODO ned spell entry?
	        if (pTarget && pTarget->getDeathState() == JUST_DIED)
		{
	        if (Creature* pSpirit = me->SummonCreature(NCP_SPIRIT_DEFENSIVE,pTarget->GetPositionX(),pTarget->GetPositionY(),pTarget->GetPositionZ(),0,TEMPSUMMON_CORPSE_DESPAWN,1000))
		pSpirit->SetReactState(REACT_AGGRESSIVE);
		}
	      }
	}
    }
...
CreatureAI* GetAI_npc_totem_binary(Creature* pCreature)
{
    return new npc_totem_binaryAI (pCreature);
}
...
void AddSC_stonetalon_mountains()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_totem_binary";
    newscript->GetAI = &GetAI_npc_totem_binary;
    newscript->RegisterSelf();
}
