...
{
    boss_omenAI(Creature *c) : ScriptedAI(c) {}

    uint32 CliveTimer;

    void Reset()
    {
       CliveTimer = 10000 + rand()%4000;
    }
    void JustDied(Unit *victim)
    {
    DoCastAOE(26392);
    //m_creature->CompletedAchievement(937);
    }
	...
	if (Unit* pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0))
	    DoCast(pTarget,40504);
	CliveTimer = 10000 +rand()%4000;
	} else CliveTimer -= diff;
	
        DoMeleeAttackIfReady();
    }
};

CreatureAI* GetAI_boss_omen(Creature* pCreature)
{
...
void AddSC_moonglade()
{
    ...
    newscript = new Script;
    newscript->Name = "boss_omen";
    newscript->GetAI = &GetAI_boss_omen;
    newscript->RegisterSelf();
}
