...
struct mob_vrykul_skeletonAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        BoneArmor_Timer = urand(15000,20000);
        isDead = false;
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        if (pInstance && pInstance->GetData(DATA_PRINCEKELESETH_EVENT) == IN_PROGRESS)
        {
            ...
            else
            {
                ...
                // addition for bone armor
                Unit* prince = Unit::GetUnit((*me), pInstance ? pInstance->GetData64(DATA_PRINCEKELESETH) : 0);
                if(prince)
                {
                    // if prince doesnt have bone armor on him we allow timer update or casting of bone armor spell
                    if(!prince->HasAura(SPELL_BONE_ARMOR))
                    {
                        if(BoneArmor_Timer <= diff)
                        {
                            // let skeleton cast a bone armor spell
                            me->AddAura(SPELL_BONE_ARMOR,prince);
                            BoneArmor_Timer = urand(15000,20000);
                        }else BoneArmor_Timer -=diff;
                    }
                    //if prince does have bone armor on him, we get new random timer setting (this is done for all alive skeletons if prince already has bone armor on himself)
                    else
                        BoneArmor_Timer = urand(15000,20000);
                }
