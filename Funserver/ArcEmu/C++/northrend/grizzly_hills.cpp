...
struct npc_harrison_jonesAI : public npc_escortAI
{
    ...
    uint64 FlamewallGUID;
    uint64 CageGUID;

    uint64 AdarrahGUID;

    Creature* Adarrah;
    GameObject* Flamewall;
    GameObject* Cage;
    ...
    void WaypointReached(uint32 uiWP)
    {
        ...
        switch (uiWP)
        {
            ...
            case 7:
                ...
                break;
            case 8:
                Cage = me->FindNearestGameObject(GO_CAGE, 50);
                Cage->SetGoState(GO_STATE_ACTIVE);
                if (Creature* pAdarrah  = GetClosestCreatureWithEntry(me, NPC_ADARRAH, 50.0f))
                            Adarrah  = pAdarrah ;
                DoScriptText(SAY_WP_4, Adarrah); 
                Adarrah->GetMotionMaster()->MovePoint(0, 4866.035f,-4789.054f,32.552f); 
                break;                   
            case 10:
                Adarrah->DisappearAndDie();
                me->HandleEmoteCommand(16);
                break;
            case 11:
                DoCast(me, SPELL_GONG_EFFECT);
                DoScriptText(SAY_WP_5, me);
                me->PlayDirectSound(4654, pPlayer);
                break;
            case 17:
                me->SetReactState(REACT_PASSIVE);
                Flamewall = me->FindNearestGameObject(GO_FLAMEWALL, 150);
                Flamewall->SetGoState(GO_STATE_READY);
                me->SummonCreature(NPC_TECAHUNA, 4907.812, -4819.371, 32.560, 2.422, TEMPSUMMON_DEAD_DESPAWN, 90000);
                break;
            case 18:
            ...
            case 20:
                Flamewall = me->FindNearestGameObject(GO_FLAMEWALL, 150);
                Flamewall->SetGoState(GO_STATE_ACTIVE);
                break;
            case 46:
                DoScriptText(SAY_WP_10, me);                
                Completed = true;
                if (pPlayer)
                    pPlayer->GroupEventHappens(QUEST_DUN_DA_DUN_TAH, me);
                break;
            case 47:
                me->DisappearAndDie();
                break;
...
bool QuestAccept_npc_harrison_jones(Player* pPlayer, Creature* pCreature, Quest const* quest)
{
    if (quest->GetQuestId() == QUEST_DUN_DA_DUN_TAH)
    {   
        DoScriptText(SAY_Q_START, pCreature, pPlayer);
        pCreature->setFaction(113);        

        if (npc_escortAI* pEscortAI = CAST_AI(npc_harrison_jonesAI, pCreature->AI()))
...
enum eSkirmisher
{
    ...
    RANDOM_SAY_1             =  -999999,        //Ahh..better..
    RANDOM_SAY_2             =  -999998,        //Whoa.. i nearly died there. Thank you, $Race!
    RANDOM_SAY_3             =  -999997         //Thank you. $Class!
...
struct npc_wounded_skirmisherAI : public ScriptedAI
{
    ...
    {        
        if (spell->Id == SPELL_RENEW_SKIRMISHER && caster->GetTypeId() == TYPEID_PLAYER
            && CAST_PLR(caster)->GetQuestStatus(12288) == QUEST_STATUS_INCOMPLETE)
        {
            ((Player *)caster)->KilledMonster(me->GetCreatureInfo(),me->GetGUID());
...
enum
{
QUEST_THE_LIQUID_FIRE_OF_ELUNE = 12166,
NPC_BLINGHTED_ELK = 26616,
NPC_RABID_GRIZZLY = 26643,
NPC_KILL_CREDIT_ELK = 27111,
NPC_KILL_CRETIT_GRIZZLY = 27112,
ITEM_LIQUID_FIRE_OF_ELUNE = 36956,
SPELL_LIQUID_FIRE_OF_ELUNE = 46770
};

bool ItemUse_item_liquid_fire_of_elune(Player* pPlayer, Item* pItem, const SpellCastTargets &pTargets)
{
    Creature* pCreature;

    if ((pPlayer->GetQuestStatus(QUEST_THE_LIQUID_FIRE_OF_ELUNE) == QUEST_STATUS_INCOMPLETE)/* && pPlayer->HasAura(SPELL_AURA_RIGHTEOUS_VISION)*/)
    {
        if (pCreature = pPlayer->FindNearestCreature(NPC_BLINGHTED_ELK, 15.0))
        {
            pPlayer->CastSpell(pCreature,SPELL_LIQUID_FIRE_OF_ELUNE,true);
            pPlayer->KilledMonsterCredit(NPC_KILL_CREDIT_ELK,0);
	    pCreature->ForcedDespawn();
        }
        else if (pCreature = pPlayer->FindNearestCreature(NPC_RABID_GRIZZLY, 15.0))
        {
             pPlayer->CastSpell(pCreature,SPELL_LIQUID_FIRE_OF_ELUNE,true);
	     pPlayer->KilledMonsterCredit(NPC_KILL_CRETIT_GRIZZLY,0);
	     pCreature->ForcedDespawn();
        }
        return false;
    }
...
struct npc_emilyAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 i)
    {
        ...
        {
            case 9:
                Mrfloppy = GetClosestCreatureWithEntry(me, NPC_MRFLOPPY, 100.0f);
                break;
            case 10:
                if (Mrfloppy)
                {
            ...
            case 17:
                ...
                RWORG = me->SummonCreature(NPC_RAVENOUS_WORG,me->GetPositionX()+10,me->GetPositionY()+8,me->GetPositionZ()+2,3.229f,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,120000);
                RWORG->setFaction(35);
                break;
            case 18:
                if (Mrfloppy)
...
void AddSC_grizzly_hills()
{
    ...
    newscript->pQuestAccept = &QuestAccept_npc_harrison_jones;
    newscript->RegisterSelf();
    ...
    newscript = new Script;
    newscript->Name = "item_liquid_fire_of_elune";
    newscript->pItemUse = &ItemUse_item_liquid_fire_of_elune;
    newscript->RegisterSelf();
