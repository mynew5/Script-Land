...
/*######
## npc_hemet_nesingwary
######*/

enum eHemetNesingwary
{
    SPELL_SAMPLE_FROM_JUNGLE		= 51962,
    QUEST_TASTE_TEST                        		= 12645,
	NPC_HEMET_NESINGWARY		= 27986,
	NPC_HADRIUS_HARLOWE		= 28047,
	NPC_TAMARA_WOBLESPROCKET	= 28568
};
...
struct npc_hemet_nesingwaryAI : public ScriptedAI
{
	...
	spell->Id == SPELL_SAMPLE_FROM_JUNGLE && 
	CAST_PLR(caster)->GetQuestStatus(QUEST_TASTE_TEST) == QUEST_STATUS_INCOMPLETE)
	{ 
	CAST_PLR(caster)->KilledMonsterCredit(NPC_HEMET_NESINGWARY,0);
	}	
    }
};
...
struct npc_hadrius_harloweAI : public ScriptedAI
{
	...
	CAST_PLR(caster)->GetQuestStatus(QUEST_TASTE_TEST) == QUEST_STATUS_INCOMPLETE)
	{ 
	CAST_PLR(caster)->KilledMonsterCredit(NPC_HADRIUS_HARLOWE,0);
	}	
    }
};
...
struct npc_tamara_wobblesprocketAI : public ScriptedAI
{
    npc_tamara_wobblesprocketAI(Creature* c) : ScriptedAI(c) {}
	...
	CAST_PLR(caster)->GetQuestStatus(QUEST_TASTE_TEST) == QUEST_STATUS_INCOMPLETE)
         { 
	CAST_PLR(caster)->KilledMonsterCredit(NPC_TAMARA_WOBLESPROCKET,0);
         }	
    }
};

CreatureAI* GetAI_npc_tamara_wobblesprocket(Creature* pCreature)
{
    return new npc_tamara_wobblesprocketAI(pCreature);
}
...
enum
{
    QUEST_SOME_MAKE_LEMONADE_SOME_MAKE_LIQUOR = 12634,
    SPELL_SUMMON_GO_ORANGE                    = 51837,
    SPELL_SUMMON_GO_BANANA_BUNCH              = 51836,
    SPELL_SUMMON_GO_PAPAYA                    = 51839,
    SPELL_SUMMON_NPC_ADVENTUROUS_DWARF        = 52070,
    NPC_FRUIT_TOSSER                          = 28466
};

bool GOHello_go_sturdy_vine(Player* pPlayer, GameObject* pGo)
{
    if (pPlayer->GetQuestStatus(QUEST_SOME_MAKE_LEMONADE_SOME_MAKE_LIQUOR) == QUEST_STATUS_INCOMPLETE)
    {
        Creature * pCreature = GetClosestCreatureWithEntry(pGo, NPC_FRUIT_TOSSER, 1.0f);
        {
            if (pCreature)
            {
                int iRandom = urand(0,3);
                switch(iRandom)
                {
                    case 0:
                      pCreature->CastSpell(pPlayer,SPELL_SUMMON_GO_ORANGE,true);
                    break;
                    case 1:
                      pCreature->CastSpell(pPlayer,SPELL_SUMMON_GO_BANANA_BUNCH,true);
                    break;
                    case 2:
                      pCreature->CastSpell(pPlayer,SPELL_SUMMON_GO_PAPAYA,true);
                    break;
                    case 3:
                      pCreature->CastSpell(pPlayer,SPELL_SUMMON_NPC_ADVENTUROUS_DWARF,true);
                    break;
                }
            }
        }
...
void AddSC_sholazar_basin()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_tamara_wobblesprocket";
    newscript->GetAI = &GetAI_npc_tamara_wobblesprocket;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "go_sturdy_vine";
    newscript->pGOHello = &GOHello_go_sturdy_vine;
    newscript->RegisterSelf();
}
