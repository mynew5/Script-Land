        ...
        if (uiPhaseTimer <= diff)
        {
            switch (Phase)
            {
                ...
                case 7:
                {
                    ...
                    break;
                case 8:
                    me->CastSpell(me, SPELL_TRAPPED, false);
                    Phase = 0;
                    break;
            }
        } else uiPhaseTimer -= diff;
...
struct mob_seaforium_depth_chargeAI : public ScriptedAI
{
...
enum eReinforcedNet
{
    ...
    QUEST_DEATH_FROM_ABOVE                                  = 11889,
    SPELL_REINFORCED_NET                                    = 46361
};

struct npc_flamespitter : public ScriptedAI
{
    npc_flamespitter(Creature *c) : ScriptedAI(c) { Reset(); }
    ...
    void SpellHit(Unit *caster, const SpellEntry *spell)
    {
        if (caster->GetTypeId() == TYPEID_PLAYER && spell->Id == SPELL_REINFORCED_NET)
        {
            uint32 credit = 0;
            switch(me->GetEntry())
            {
                case NPC_FLAMESPITTER:
                    credit = NPC_FLAMESPITTER;
                   break;
                default: 
                    break;
            }
            ...
            if(credit)
            {
                ((Player*) caster)->DealDamage(me, me->GetHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
            }
        }
    }
...
CreatureAI* GetAI_npc_flamespitter(Creature* pCreature)
{
    return new npc_flamespitter (pCreature);
}
...
enum
{
ITEM_GNOMISH_GRENADE	= 34772,
SPELL_BOMB	= 45583,
QUEST_HAMPERING_THEIR_ESCAPE = 11617,
NPC_EAST_PLATFORM_DESTROET = 25442,
NPC_NORTH_PLATFORM_DESTROET = 25441,
NPC_WEST_PLATFORM_DESTROET = 25443
};

bool ItemUse_item_gnomish_grenade(Player* pPlayer, Item* pItem, const SpellCastTargets &pTargets)
{
    Creature* pCreature;

    if ((pPlayer->GetQuestStatus(QUEST_HAMPERING_THEIR_ESCAPE) == QUEST_STATUS_INCOMPLETE)/* && pPlayer->HasAura(SPELL_AURA_RIGHTEOUS_VISION)*/)
    {
        if (pCreature = pPlayer->FindNearestCreature(NPC_EAST_PLATFORM_DESTROET, 15.0))
        {
            pPlayer->CastSpell(pCreature,SPELL_BOMB,true);
            pPlayer->KilledMonsterCredit(NPC_EAST_PLATFORM_DESTROET,0);
	    pCreature->ForcedDespawn();
        }
        else if (pCreature = pPlayer->FindNearestCreature(NPC_NORTH_PLATFORM_DESTROET, 15.0))
        {
             pPlayer->CastSpell(pCreature,SPELL_BOMB,true);
	     pPlayer->KilledMonsterCredit(NPC_NORTH_PLATFORM_DESTROET,0);
	     pCreature->ForcedDespawn();
        }
        else if (pCreature = pPlayer->FindNearestCreature(NPC_WEST_PLATFORM_DESTROET, 15.0))
        {
             pPlayer->CastSpell(pCreature,SPELL_BOMB,true);
	     pPlayer->KilledMonsterCredit(NPC_WEST_PLATFORM_DESTROET,0);
	     pCreature->ForcedDespawn();
        }
        return false;
    }
...
#define GOSSIP_ITEM_FIZZCRANK "I am ready"
enum
{
SPELL_SUMMON_PILOTS_INSIGMA = 46166,
QUEST_EMERGENCY_PROTOKOL_PARAGRAFC = 11795,
NPC_FIZZCRANK_RECON_PILOT = 25841
};
bool GossipHello_npc_fizzcrank_recon_pilot(Player* pPlayer, Creature* pCreature)
{

    if (pPlayer->GetQuestStatus(QUEST_EMERGENCY_PROTOKOL_PARAGRAFC) == QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_FIZZCRANK, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);

    pPlayer->SEND_GOSSIP_MENU(pPlayer->GetGossipTextId(pCreature), pCreature->GetGUID());
    return true;
}

bool GossipSelect_npc_fizzcrank_recon_pilot(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    switch (uiAction)
    {
        case GOSSIP_ACTION_INFO_DEF+1:
            pPlayer->CastSpell(pPlayer, SPELL_SUMMON_PILOTS_INSIGMA, true);
            pPlayer->CLOSE_GOSSIP_MENU();
	    pCreature->ForcedDespawn();
            break;
...
void AddSC_borean_tundra()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_flamespitter";
    newscript->GetAI = &GetAI_npc_flamespitter;
    newscript->RegisterSelf();
    ...
    newscript = new Script;
    newscript->Name = "item_gnomish_grenade";
    newscript->pItemUse = &ItemUse_item_gnomish_grenade;
    newscript->RegisterSelf();
    
    newscript = new Script;
    newscript->Name = "npc_fizzcrank_recon_pilot";
    newscript->pGossipHello = &GossipHello_npc_fizzcrank_recon_pilot;
    newscript->pGossipSelect = &GossipSelect_npc_fizzcrank_recon_pilot;
    newscript->RegisterSelf();
}
