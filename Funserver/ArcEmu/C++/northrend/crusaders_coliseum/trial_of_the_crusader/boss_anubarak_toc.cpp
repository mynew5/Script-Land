...
struct  boss_anub_arak_crusaderAI : public ScriptedAI
{
	...
	if (!UpdateVictim())
		return;
			...
			{
				me->SummonCreature(NPC_FROST_SPHERE, 750.183, 172.536, 164.498, 0, TEMPSUMMON_CORPSE_DESPAWN, 120000);
				me->SummonCreature(NPC_FROST_SPHERE, 758.665, 142.486, 168.611, 0, TEMPSUMMON_CORPSE_DESPAWN, 120000);
				me->SummonCreature(NPC_FROST_SPHERE, 763.031, 103.666, 172.110, 0, TEMPSUMMON_CORPSE_DESPAWN, 120000);
						SUMMON_FROSTSPHERE_Timer = 20000;
			...
			if (Event == true)
			{
					if (SUMMON_SCARAB_Timer <= diff)
					{
						...
						if (Creature *pBoss=me->SummonCreature(NPC_SCARAB, 731.482, 80.675,142.190, 0, TEMPSUMMON_CORPSE_DESPAWN, 120000))
						{
							if (Unit* pTarget = SelectUnit(SELECT_TARGET_RANDOM,0))
							pBoss->AI()->AttackStart(pTarget);
						}
						if (Creature *pBoss=me->SummonCreature(NPC_SCARAB, 727.973, 189.955, 142.119, 0, TEMPSUMMON_CORPSE_DESPAWN, 120000))
						{
							if (Unit* pTarget = SelectUnit(SELECT_TARGET_RANDOM,0))
							pBoss->AI()->AttackStart(pTarget);
						}
						if (Creature *pBoss=me->SummonCreature(NPC_SCARAB, 686.757, 97.445,142.119, 0, TEMPSUMMON_CORPSE_DESPAWN, 120000))
						{
							if (Unit* pTarget = SelectUnit(SELECT_TARGET_RANDOM,0))
							pBoss->AI()->AttackStart(pTarget);
						}
						if (Creature *pBoss=me->SummonCreature(NPC_SCARAB, 692.077, 184.421, 142.119, 0, TEMPSUMMON_CORPSE_DESPAWN, 120000))
						{
							if (Unit* pTarget = SelectUnit(SELECT_TARGET_RANDOM,0))
							pBoss->AI()->AttackStart(pTarget);
						}
		...
		{
			    case 3:
					if (Event75 != false && (me->GetHealth()*100 / me->GetMaxHealth()) < 75)
						Phase = 0;
						Event = true;
					if (Event50 != false && (me->GetHealth()*100 / me->GetMaxHealth()) < 50)
						Phase = 0;
						Event = true;
					if (Event25 != false && (me->GetHealth()*100 / me->GetMaxHealth()) < 25)
						Phase = 0;
						Event = true;
					...
					{
						DoScriptText(SAY_STAYA, me);
						if (Creature *pBoss=me->SummonCreature(NPC_BURROWER, 686.757, 97.445,142.119, 0, TEMPSUMMON_CORPSE_DESPAWN, 120000))
						{
							if (Unit* pTarget = SelectUnit(SELECT_TARGET_RANDOM,0))
							pBoss->AI()->AttackStart(pTarget);
						}
						if (Creature *pBoss=me->SummonCreature(NPC_BURROWER, 731.482, 80.675,142.190, 0, TEMPSUMMON_CORPSE_DESPAWN, 120000))
						{
							if (Unit* pTarget = SelectUnit(SELECT_TARGET_RANDOM,0))
							pBoss->AI()->AttackStart(pTarget);
						}
	...
	if (Phase_Timer < diff)
	{
		switch(Phase)
		{
				...
				case 1:
					...
					{
							DoCast(me->getVictim(), SPELL_MARK);
...
struct  npc_borrowerAI : public ScriptedAI
{
	...
	me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
	DoCast(me, SPELL_SUBMERGE_M, false);
	Submerge_Timer = 35000;
	mobSubmerged = true;
	} 
	else 
