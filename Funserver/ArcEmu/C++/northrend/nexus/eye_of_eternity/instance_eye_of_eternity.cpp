...
struct instance_eye_of_eternity : public ScriptedInstance
{
    ...
    void OnPlayerEnter(Player* pPlayer)
    {
        ...
        uint32 vehicleHealth = 1;
        Vehicle *pTemp = NULL;
        switch(m_uiInstanceEnterRules)
        {
            case DATA_ALLOW_ENTER_VEHICLE:
                pTemp = (Vehicle*)pPlayer->SummonCreature(NPC_WYRMREST_SKYTALON, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 0);
                if(!pTemp) // TODO
                    return;
                ((Creature*)pTemp)->SetCreatorGUID(pPlayer->GetGUID());
                vehicleHealth = 100000 + (pPlayer->GetMaxHealth()*2); // may be wrong
                ((Creature*)pTemp)->SetMaxHealth(vehicleHealth);
                ((Creature*)pTemp)->SetHealth(vehicleHealth);
