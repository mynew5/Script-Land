...
/*######
# npc_Experiencie_eliminator
######*/

#define GOSSIP_TEXT_1	"I don't want receive an experience."
#define GOSSIP_TEXT_2	"I want receive experience again."

bool GossipHello_npc_experience_eliminator(Player *pPlayer, Creature *pCreature)
{
	if(!pPlayer->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_NO_XP_GAIN))
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_TEXT_1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
	else
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_TEXT_2, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
	pPlayer->SEND_GOSSIP_MENU(14736, pCreature->GetGUID());
	return true;
}
...
/*######
## npc_spring_rabbit + for achievement #2497
######*/
...
struct npc_spring_rabbitAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        if (!isLove)
        {
            ...
            {
                Trinity::AnyFriendlyUnitInObjectRangeCheck u_check(me, me, 8.0f);
                Trinity::UnitListSearcher<Trinity::AnyFriendlyUnitInObjectRangeCheck> searcher(me, tempUnitMap, u_check);
                ...
                cell.Visit(p, world_unit_searcher, *me->GetMap(), *me, 8.0f);
                cell.Visit(p, grid_unit_searcher, *me->GetMap(), *me, 8.0f);
            }
	    
            for (std::list<Unit*>::const_iterator i = tempUnitMap.begin(); i != tempUnitMap.end(); i++)
            {
	       if ((*i)->GetTypeId() == TYPEID_UNIT && (*i)->GetEntry() == 32791 && (*i)->GetGUID() != me->GetGUID())
		{
		       DoCast(*i,61728);
		    if (Unit *owner = me->GetOwner())
