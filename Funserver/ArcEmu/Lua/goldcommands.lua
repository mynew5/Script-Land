local MSG_COMMAND = "#gold commands"

function GoldCommandsOnChat(event, player, message, type, language)
	if(message == MSG_COMMAND) then
		...
		player:SendBroadcastMessage("|cFF00FFFFGold Gear: |cFFFFA500 Type: #gold - Gives you full Gold VIP gear!")
		...
		player:SendBroadcastMessage("|cFF00FFFFIncrease Honor: |cFFFFA500 Type: #increase honor - Adds your char +2500 honor!")
		...
		player:SendBroadcastMessage("|cFF00FFFFcommands: |cFFFFA500 Type: .commands - to see all the GM commands GOLD has unlocked!")
...
RegisterServerHook(16, "GoldCommandsOnChat")
