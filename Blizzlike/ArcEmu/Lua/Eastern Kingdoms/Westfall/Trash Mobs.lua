...
function DHW_Backstab(Unit, Event)
	if(Unit:GetMainTank() ~= nil) then
		Unit:FullCastSpellOnTarget(53, Unit:GetMainTank())
	end
end
...
function DPS_SBash(Unit, Event)
	if(Unit:GetRandomPlayer(4) ~= nil) then
		Unit:FullCastSpellOnTarget(11972, Unit:GetRandomPlayer(4))
	end
end
...
function DTR_OnCombat(Unit, Event)
		Unit:SendChatMessage(12, 0, "Ah , a chance to use this freshly sharpened blade")
...
function DTR_BSone(Unit, Event)
	if (Unit:GetMainTank() ~= nil) then
		Unit:FullCastSpellOnTarget(2589, Unit:GetMainTank())
	end
end
...
function FLS_COS(Unit, Event)
	if(Unit:GetMainTank() ~= nil) then
		Unit:FullCastSpellOnTarget(3105, Unit:GetMainTank())
	end
end

function FLS_PolySheep(Unit, Event)
	if(Unit:GetMainTank() ~= nil) then
		Unit:FullCastSpellOnTarget(118, Unit:GetMainTank())
	end
end
...
function Har_WSlashh(Unit, Event)
	if(Unit:GetMainTank() ~= nil) then
		Unit:FullCastSpellOnTarget(7342, Unit:GetMainTank())
	end
end
...
function MCT_PStab(Unit, Event)
	if(Unit:GetMainTank() ~= nil) then
		Unit:FullCastSpellOnTarget(7357, Unit:GetMainTank())
	end
end
...
function RPMM_SDcay(Unit, Event)
	if(Unit:GetMainTank() ~= nil) then
		Unit:FullCastSpellOnTarget(8016, Unit:GetMainTank())
	end
end
...
function SBlaw_Kck(Unit, Event)
	if(Unit:GetMainTank() ~= nil) then
		Unit:FullCastSpellOnTarget(5164, Unit:GetMainTank())
	end
end
